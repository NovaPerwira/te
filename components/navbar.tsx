      ...
      >
        ...
        <Link href="/" className="z-50 group">
          <h1 className="text-2xl font-black uppercase tracking-tighter flex items-center gap-1">
            Nova Perwira
            <span className="w-2 h-2 bg-black rounded-full group-hover:bg-sky-600 transition-colors"></span>
          </h1>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className="text-sm font-bold uppercase tracking-widest hover:text-sky-600 transition-colors relative group"
            >
              {link.name}
